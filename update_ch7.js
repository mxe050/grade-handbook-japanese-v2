...
// Regex for placeholders (match the entire block)
const p1 = /<div class="placeholder-msg">[\s\S]*?<h3>CoreGrade解説<\/h3>[\s\S]*?<\/div>/;
const p2 = /<div class="placeholder-msg">[\s\S]*?<h3>CoreGRADEフローチャート<\/h3>[\s\S]*?<\/div>/;

// Replace only the first occurrence for both
content = content.replace(p1, tab1_content);
content = content.replace(p2, tab2_content);

fs.writeFileSync(path, content, 'utf8');
console.log('Successfully updated Chapter 7 placeholders using Node.js.');
